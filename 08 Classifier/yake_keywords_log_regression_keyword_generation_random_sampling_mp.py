...
def extract_yake_keywords(text):
    ...
    keyword_yake_noun_chunk = [(" ".join([lemmatizer.lemmatize(word.replace('-', ' ')).lower() for word in keyword.split()]), score) for keyword, score in keyword_yake_noun_chunk]
    keyword_yake_noun_chunk = [(re.sub(r"[^a-zA-Z0-9 ]", "", keyword).lower().strip(), score) for keyword, score in keyword_yake_noun_chunk]
    return keyword_yake_noun_chunk

def postprocess_keyword_list(keyword_yake_noun_chunk):
    keyword_yake_noun_chunk = [keyword for keyword, score in keyword_yake_noun_chunk if score <= 0.1]
    filtered_keywords = []
    for keyword in keyword_yake_noun_chunk:
        words = keyword.split()
        if all(word.isalnum() or word in ["-", "'"] for word in words) and len(keyword) >= 3:
            lemmatized_keyword = " ".join([lemmatizer.lemmatize(word) for word in words])
            if lemmatized_keyword not in stopwords:
                filtered_keywords.append(lemmatized_keyword)
    filtered_keywords = list(set(filtered_keywords))
    filtered_keywords = [keyword for keyword in filtered_keywords if not re.fullmatch(r'\b(?:[A-Z]{1,}\.){2,}\b|\b[A-Z]{1,3}\b', keyword)]
