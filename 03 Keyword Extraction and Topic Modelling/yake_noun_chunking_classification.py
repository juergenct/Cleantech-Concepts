...
# Only keep columns 'cpc_classification', 'sequence', 'title', 'full_title'
df = df[['cpc_classification', 'sequence', 'title', 'full_title']]

# Initialize keywords_yake column
df['keywords_yake_title'] = ''
df['keywords_yake_title_noun_chunk'] = ''
df['noun_chunks'] = ''
...
for index, row in tqdm(df.iterrows()):
    try:
        if row['title'] is None:
            df.at[index, 'keywords_yake_title'] = [], []
            df.at[index, 'keywords_yake_title_noun_chunk'] = []
        
        # Normalize the text with unicodedata
        row['title'] = unicodedata.normalize("NFKD", row['title']).encode('ASCII', 'ignore').decode('utf-8')
        # row['title'] = re.sub(r"[^a-zA-Z- ]|^https?:\/\/.*[\r\n]*|\[.*?\]|\(.*?\)|\{.*?\}", " ", row['title']).lower().strip()
        row['title'] = re.sub(r"\[.*?\]|\(.*?\)|\{.*?\}", "", row['title'])
        row['title'] = re.sub(r"https?:\/\/\S+", "", row['title'])
        row['title'] = re.sub(r"[^a-zA-Z0-9- .,;!?]", "", row['title'])
        unfiltered_keywords = yake_extractor.extract_keywords(row['title']) # Extract keywords using YAKE
        doc = nlp(row['title'])
        ...
        df.at[index, 'keywords_yake_title_noun_chunk'] = filtered_keywords
